...
public class Boss : EnemyBase
{
    ...
    // 敵召喚攻撃開始
    private void StartEnemySummonAttack()
    {
        if (!enemyPrefab1 || !enemyPrefab2)
        {
            Debug.LogWarning(
                "enemyPrefabが設定されていません"
            );
        ...
        if (enemySpawnPointList == null ||
            enemySpawnPointList.Count < 2)
        {
            Debug.LogWarning(
                "enemySpawnPointListに2つ以上のSpawnPointを設定してください"
            );
    ...
    public void SpawnEnemy()
    {
        ...
        {
            Debug.LogWarning(
                "Enemy Prefab 1 または Enemy Prefab 2 が設定されていません"
            );
        ...
        GameObject enemy1 = Instantiate(
            enemyPrefab1,
            enemySpawnPointList[0].position,
            enemySpawnPointList[0].rotation
        );

        GameObject enemy2 = Instantiate(
            enemyPrefab2,
            enemySpawnPointList[1].position,
            enemySpawnPointList[1].rotation
        );

        // 召喚した敵を記録
        summonedEnemies.Add(enemy1);
        summonedEnemies.Add(enemy2);

        Debug.Log("Boss：敵を2体召喚");
    }
